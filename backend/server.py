...
async def record_swipe(swipe: SwipeAction, current_user: dict = Depends(get_current_user)):
    """
    Record a swipe action (like, pass, superlike) and check for mutual matches
    """
    # Validate action
    if swipe.action not in ['like', 'pass', 'superlike']:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Invalid action. Must be 'like', 'pass', or 'superlike'"
        )
    
    # Check if target user exists
    target_user = await db.users.find_one({"id": swipe.target_user_id}, {"_id": 0})
    if not target_user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    existing_swipe = await db.swipes.find_one({
        "user_id": current_user['id'],
        "swiped_user_id": swipe.target_user_id
    })
    ...
    # Check for mutual match if action is 'like' or 'superlike'
    matched = False
    if swipe.action in ['like', 'superlike']:
        ...
        if mutual_like:
            ...
            existing_match = await db.matches.find_one({
                "$or": [
                    {"user1_id": current_user['id'], "user2_id": swipe.target_user_id},
                    {"user1_id": swipe.target_user_id, "user2_id": current_user['id']}
                ]
            })
            ...
            if not existing_match:
                ...
                # Optionally send notifications (stub for now)
                # await send_match_notification(current_user['id'], swipe.target_user_id)
    
    return {
        "status": "success",
        "action": swipe.action,
        "matched": matched,
        "message": "It's a match! 🎉" if matched else f"Swipe {swipe.action} recorded"
    }
